# Databricks notebook source exported at Mon, 10 Oct 2016 09:23:32 UTC
# MAGIC %md
...
def parse_points(df):
    ...
    """
    return df.select(sql_functions.split(df.value, ',')).map(lambda values : LabeledPoint(float(values[0][0]), values[0][1:])).toDF()
...
# TODO: Replace <FILL IN> with appropriate code
content_stats = (parsed_points_df.selectExpr('min(label)', 'max(label)')).collect()
...
.select(parsed_points_df.features, parsed_points_df.label-min_year)
...
weights = [.8, .1, .1]
seed = 42
parsed_train_data_df, parsed_val_data_df, parsed_test_data_df = parsed_data_df.randomSplit([.8,.1,.1],42)
...
def get_labeled_prediction(weights, observation):
    ...
    """
    return (weights.dot(observation.features),observation.label)
...
def linreg_gradient_descent(train_data, num_iters):
    ...
    for i in range(num_iters):
        ...
        preds_and_labels_train = train_data.map(lambda x : get_labeled_prediction(w,x)).map(lambda x: (float(x[0]),float(x[1])))
        preds_and_labels_train_df = sqlContext.createDataFrame(preds_and_labels_train, ["prediction", "label"])
...
preds_and_labels = (parsed_val_data_df
                      .map(lambda lp: get_labeled_prediction(weights_LR0,lp)).map(lambda x: (float(x[0]),float(x[1]))))
preds_and_labels_df = sqlContext.createDataFrame(preds_and_labels, ["prediction", "label"])
rmse_val_LR0 = calc_RMSE(preds_and_labels_df)

print preds_and_labels.first()
print preds_and_labels.count()
