# Databricks notebook source exported at Mon, 10 Oct 2016 09:23:41 UTC
# MAGIC %md
...
sample_data_df.show(n = 2, truncate = False)
...
sample_distinct_feats_df = (sample_data_df
                              .select(explode(sample_data_df.features).alias("feature_id")).distinct())
...
def create_one_hot_dict(input_df):
    ...
    Returns:
        dict: A dictionary where the keys are (featureID, value) tuples and map to values that are
            unique integers.
    """
    return input_df.select(explode(input_df.features).alias("feature_id")).distinct().map(lambda r: tuple(r[0])).zipWithIndex().collectAsMap()
...
n_train = raw_train_df.cache()
n_val = raw_validation_df.cache()
n_test = raw_test_df.cache()
...
def one_hot_encoding(raw_feats, ohe_dict_broadcast, num_ohe_feats):
    ...
    Returns:
        SparseVector: A SparseVector of length num_ohe_feats with indices equal to the unique
            ...
            with values equal to 1.0.
    """
    list = [ohe_dict_broadcast.value[f] for f in raw_feats if f in ohe_dict_broadcast.value]
    sortedlist = sorted(list)
    valuelist = [1 for f in raw_feats if f in ohe_dict_broadcast.value ]
    return SparseVector(num_ohe_feats, sortedlist, valuelist) 
  
parsed_validation_df = parse_raw_df(raw_validation_df)
ohe_dict_missing_udf = ohe_udf_generator(ohe_dict_broadcast)
ohe_validation_df = (parsed_validation_df.select(parsed_validation_df.label, ohe_dict_missing_udf(parsed_validation_df.feature).alias('features')))
ohe_validation_df.cache()
ohe_validation_df.printSchema()
print ohe_validation_df
...
def add_log_loss(df):
    ...
    """
    return df.select("*", when(df.label == 1, 0. - log(df.p + epsilon)).\
