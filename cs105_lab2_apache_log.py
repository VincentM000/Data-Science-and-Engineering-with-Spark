# Databricks notebook source exported at Mon, 10 Oct 2016 09:21:22 UTC
# MAGIC %md
...
days_with_hosts = daily_hosts_df.select('day').distinct().collect()
hosts = daily_hosts_df.select('count').collect()
for index in range(len(days_with_hosts)):
  hosts[index]= hosts[index][0]
  days_with_hosts[index] = days_with_hosts[index][0]
...
avg_daily_req_per_host_df = (
  total_req_per_day_df.join(daily_hosts_df, total_req_per_day_df.day == daily_hosts_df.day)
                      .select(total_req_per_day_df['day'], (total_req_per_day_df['count']/daily_hosts_df['count']).alias('avg_reqs_per_host_per_day'))
).cache()
...
for index in range (len(days_with_avg)):
  avgs[index] = avgs[index][0]
  days_with_avg[index] = days_with_avg[index][0]
...
errors_by_date_sorted_df.show(21)
...
errors_404_by_day = []
for a,b in errors_by_date_sorted_df.collect():
  errors_404_by_day.append(b)
  days_with_errors_404.append(a)
...
hours_with_not_found = [d['hour'] for d in hour_records_sorted_df.select('hour').collect()]
not_found_counts_per_hour = [d['count'] for d in hour_records_sorted_df.select('count').collect()]
